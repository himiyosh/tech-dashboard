...
- 広い Zenn AI feed の Local LLM 判定をタイトル根拠へ限定し、本文中の「ローカルのプロジェクトフォルダ」のような偶発語だけを理由に、業務向け AI デモが `local-llm` として掲載されないようにしました。既存の live/archive data にも同じ source filter を再適用しています。
