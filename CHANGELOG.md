...
- sparseまたはtitle-onlyのsource inputでは十分な収集元contextがない限りAI要約・解説本文を生成せず、公式title/snippetと料金plan・対象地域・platform展開がmaterially矛盾する生成結果をsummary/body cache、Publisher、sidecarの全経路で拒否するようにしました。不合格cacheは再生成対象へ戻し、記事は収集元の抜粋を伴う準備待ち状態に留まります。
