...
- PR CIでexact-head独立レビューgateを実行し、event由来のPR番号・headとREST由来のcurrent open state、外部reviewer session、統括session、厳格なmarkerをfail-closedで検証するようにしました。closed済みPRのhistorical rerunだけはREST state確認後にskipし、open PRのmarker投稿後は同一headのfailed runを再実行します。CI成功後もmerge直前のlocal gateは必須です。
