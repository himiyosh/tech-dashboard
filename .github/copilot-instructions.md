...
- **対策**: explicit `clusterId`を優先し、fallbackでは収集元の原題で発表語とversion付きmodel identityが近接するheadlineだけを保守的に同一topicへ正規化する。生成titleの言い換えは採否へ使わず、原題に料金、移行、チュートリアル、分析などの実用・評価intentがある場合もclusterしない。Pro/Flash Cyber、mini、Coder/VL、model sizeなどversion後の複合variantもtopic identityへ含め、modelを利用する機能発表はmodel launchに含めない。SpotlightのID/source/topicを除外してから候補上限を適用し、Top 3内もtopicを重複させず、source/category制約を緩和するfallbackでもtopic制約だけは維持する。実Opus 5の3表現、料金・分析記事、別variant、model機能発表の非cluster、現行dataの4枠topic uniquenessをunit/E2Eへ固定する。
