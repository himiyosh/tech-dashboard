...
- `wrangler deploy` 成功後は `node scripts/verify-worker-deploy.mjs` で fingerprint の edge 伝播を bounded polling 確認する (LL-407)。deploy 自体は既に成功しているため、この確認は push をブロックしない。`SKIP_WORKER_DEPLOY_VERIFY=1` でスキップ可能。
...
- explore shortcut は **direct shortcut** であり、`#site-menu` (`navItems`) には **含めない** (LL-054: direct shortcut を menu に重複表示しない)。`navItems` に置く secondary destination は `Timeline` / `Glossary` / `Archive` / `Status` / `About` / `Privacy` + `Search` action のみ。`Glossary` (`/glossary`, AI/LLM 用語集) は鮮度減衰しないキュレーション知見なので explore shortcut / mobile tabbar には出さず、menu-owned の secondary destination にする (Glossary ページでは `Menu` trigger が active になる)。`Privacy` は consent prompt を閉じた後もmobileから広告選択・current-browser data削除へ戻れる永続controlとしてmenu-ownedにする。
...
- mobile bottom tabbar は `Home / Categories / arXiv / Knowledge / Menu` の 5 action とする。explore shortcut (Categories / arXiv / Knowledge) は desktop header と mobile tabbar の両方に出し、どちらの viewport でも到達できるようにする。Search / Archive / Status / About / Privacy は `#site-menu` に集約する。
...
- nav 変更時は E2E で desktop 3 explore shortcut 表示、desktop hamburger 表示、menu 内 secondary links (Archive / Status / About / Privacy / Search)、menu 内 Categories / arXiv / Knowledge 非表示 (重複なし)、mobile tabbar 5 item と各 item 幅・横スクロールなし、Categories / Knowledge ページで該当 shortcut が active かつ Menu 非 active、menu open/close、Archive / About / Status / Privacy / Search の menu 動線を必ず検証する。
...
- source-level `evergreen` はretention候補の指定であり、そのsourceの全記事をKnowledgeへ自動掲載する許可ではない。`web/src/lib/knowledge-eligibility.ts`の単一helperがraw `title` / `contentSnippet` / source contextだけでentry単位の適格性を判定し、一般提供・preview・月次告知・award・event登録などの告知専用記事は`knowledgeEligible:false`をstampしてKnowledge HTML/RSSから除外する。告知形式のtitleでもraw snippetにsetup、API parameter、CLIとproduction-readinessなどの狭いprocedural evidenceがある場合はKnowledgeへ維持する。availability acronymの`GA`は大文字の独立tokenだけを認識し、小文字`ga`や元素記号`Ga`を除外根拠にしない。生成`titleJa`/`titleEn`/`summaryJa`/`summaryEn`を採否へ使わない。Publisher normalize/restamp、`scripts/clean-source-noise.mjs`、Web、Knowledge RSSは同じhelperを共有する。除外記事のsource-level evergreen retention、Timeline、カテゴリ、検索、Archive、canonical URL、要約、本文sidecarは維持する。
...
- evergreen 知見は news Timeline とは別の `/knowledge` レーンに蓄積表示する。`web/src/lib/data.ts` の `KNOWLEDGE_ENTRIES` (`isKnowledgeEligibleEntry`) と `knowledgeBySource()` を単一情報源にし、ソース別グルーピングで出す。ページは header の `Knowledge` explore shortcut と mobile tabbar の `Knowledge` タブから遷移する (R-015)。hot 期間の新着は Timeline にも出るが、恒久の置き場は `/knowledge`。evergreen ソースまたはentry適格性を増減したら `/knowledge` のHTML/RSS parityとE2E (header/tabbar の Knowledge shortcut 動線・ソース別グループ・menu 内 Knowledge 非重複) を通す。
...
- **事象**: 62 filesを含むmerge commitのpushで、pre-push hookは`git diff --name-only`から17件のWeb影響fileを得られる状態なのに`web/への影響なし`としてPublisher E2Eをskipした。同じSHA rangeをpush後に直接検査すると17件が一致した。後日の別レビューで、同じhook内の`worker/`差分判定 (`git diff --name-only "$range" -- worker/ | grep -q .`) と、`pre-commit`のstaged TypeScript file判定 (`git diff --cached --name-only | grep -qE '\.(ts|tsx)$'`) にも同型のpipeが残っていることが見つかった。さらに別レビューで、`git diff --quiet`への置き換え後も`if git diff --quiet ...; then (差分無し) fi`という2分岐 (exit 0 vs それ以外) のままだったため、`$range`が不正 (存在しないref等) でexit 128になるエラーも「差分あり」として`RUN_WORKER_DEPLOY=1`下で誤ってdeployへ進み得ることが見つかった。
- **根本原因**: `set -o pipefail`下で`echo "$CHANGED" | grep -qE ...`を使っていた。`grep -q`が先頭matchで終了した後、大きな`echo`がSIGPIPEになるとpipeline全体がnonzeroになり、先頭の`!`が「matchなし」へ反転した。短いfile listではproducerが先に完了して再現しない。同じpipe shape (`git diff ... | grep -q...`) は、fileの種類・判定方法が違うだけで同じhookファイル内、さらに別のhookファイルにも独立に存在し得る。加えて`git diff --quiet`はexit 0 (差分無し)・1 (差分有り)・それ以外 (128等、range不正やobject読み取り失敗などのerror) の3値を返すが、「0か否か」の2分岐判定は「差分有り」と「判定自体のerror」を区別できず、error時にdeployへ進む fail-open になっていた。
- **対策**: producer pipeを廃止し、`grep -qE ... <<<"$CHANGED"`で同一process inputを渡す。`worker/`差分判定はfile一覧が不要なため、pipeを完全に避けられる`git diff --quiet "$range" -- worker/`に置き換えた。`pre-commit`のTS file判定は`staged_files=$(git diff --cached --name-only)`でcaptureしてから`grep -qE '\.(ts|tsx)$' <<<"$staged_files"`へ渡す、同じcapture-then-here-stringパターンにした。`worker/`差分判定はさらに`worker_diff_status=0; git diff --quiet "$range" -- worker/ || worker_diff_status=$?`でexit codeを明示captureし、`-eq 0`ならskip、`-ne 1`(0でも1でもない)ならERRを出してpush/deployをfail-closedで中断、それ以外 (=1) だけがdeployへ進む3分岐にした。stderrは`2>/dev/null`で握りつぶさず、git自体のfatalな診断 (`bad revision`等) がそのまま見えるようにした。source contract testでhere-string使用/`git diff --quiet`使用・3分岐の存在・旧`echo | grep -q`・旧pipe・旧2分岐の不在を固定し、実temp git repoでexit 0/1/128相当の3状態を再現するintegration testも追加して、次のWeb影響pushでPublisher E2Eが実行されることをhook出力で確認する。
- **教訓**: `pipefail`と早期終了するconsumerを組み合わせると、match成功でもproducerのSIGPIPEがpipeline失敗へ見える。判定用の小さな文字列はpipeせずhere-stringまたはfile inputを使い、短いfixtureだけでなく多数fileのmerge rangeでも実行経路を検証する。**1箇所でこのpipe shapeを見つけたら、同じhookファイル内と兄弟hookファイルの両方を`grep`で横断検索し、同型の`git diff ... | grep -q`が他に残っていないか確認する** (whack-a-moleにしない)。単純な「diffが存在するか」の判定には、file一覧を経由せず`git diff --quiet`のようなpipe不要のsubcommandを優先する。**ただし`git diff --quiet`のような「0/1/それ以外」の3値exit codeを持つcommandを`if cmd; then ... fi`の2分岐だけで判定すると、「否定条件が成立」と「判定自体が失敗した」を区別できず、error時に安全側 (fail-closed) ではなく危険側 (fail-open) へ倒れ得る**。判定対象commandが2値以上のexit codeを返し得る場合は、exit codeを変数へ明示captureし、想定外の値をdeploy等の破壊的操作へは進めずfail-closedで扱う分岐を必ず用意する。
...
### LL-406: 全feed一括分類のsourceはtitle+contentSnippetのINCLUDE/EXCLUDEルールで分割し、live+archive全corpusで検証する
- **事象**: `github-changelog` (category固定 `copilot`) と `google-developers` (category固定 `gemini`) がfeed全体を単一categoryへ一括分類していたため、Xcode runner、SSO/Settings、secret scanning、Dependabot、Visual Studio Subscription、GitHub Code Quality、GHES、npmのような無関係なGitHubプラットフォーム記事や、Google Pay決済・Sign in with GoogleセッションのようなGoogleの非AI記事が、それぞれCopilot/Geminiとして表示されていた。
- **根本原因**: `SourceDefinition.category`はsource単位の固定値で、`resolveCategory()`は`huggingface-blog`/`qiita-vscode`にだけper-entry override分岐を持っていたが、`github-changelog`/`google-developers`には無かった。両sourceとも実体は「複数トピックを扱うfeed」で、feed自体に`<category>`タグも無いため、title+contentSnippetのkeyword判定で分割する以外に手段が無かった。
- **対策**: `resolveCategory()`へ2branchを追加。`github-changelog`はcopilot/AI関連keyword (`copilot|gpt|gemini|gemma|claude|kimi|grok|openai|anthropic|mai-code|mcp|agents?|agentic|llm|opus|sonnet|haiku|ai credit|ai usage|ai adoption|model selection|models? in auto`) を要求するINCLUDE方式 (未match時はtech-news)、`google-developers`はGoogle Pay/Sign in with Google等の少数EXCLUDE方式 (未matchはgemini維持) にした。両方式ともlive+archive全192/58件のdeduped corpusで実titleを分類し、境界事例 (Claude Opus codename、AI credit、MCP entry内のGoogle Pay言及等) を手動検証してから確定した。既存artifactは`scripts/clean-source-noise.mjs --apply`で再分類し、canonical URL損失0・body損失0を確認した。`llm/llms`等7組のtag単数複数ゆれも`TAG_ALIASES`へ追加し、既存precedent (`agents->agent`, `benchmarks->benchmark`) と同様に多数派表記を正準形にした。
- **教訓**: feed全体を1つのcategoryへ固定すると、feedの話題が広いほど誤分類が蓄積する。`resolveCategory()`のper-source override分岐は既存のhuggingface-blog/qiita-vscode patternを踏襲し、新規sourceでも同じ場所に実装する。keyword判定はtitleのみ残る古いarchive entry (`contentSnippet`欠落) と、contentSnippetを持つ新しいlive entryの両方で機能する必要があるため、fixtureはlive50件だけでなくlive+archive全corpusをcanonical URLでdedupeしてから作る。tag alias同様、INCLUDE/EXCLUDEどちらの方式を選ぶかはfeedの話題分布 (大半が無関係 vs 大半が関連) で決め、既存artifactのmigrationとcanonical URL/body loss=0の検証を同じ変更に含める。

### LL-407: Worker deploy の edge propagation は immediate な /health 確認では判定できない
- **事象**: `tech-dashboard-harness` へ新 fingerprint (`sha256:32e93f...`) を deploy した直後、`wrangler deployments list` は当該 version を 100% と報告し、local dry-run bundle も新 fingerprint を含んでいたが、cache-busting query 付きの immediate `/health` 確認は依然として旧 fingerprint (`sha256:d5e68...`) を返し続けた。60 秒待って 5 回連続で確認したところ、すべて新 fingerprint に揃った。
- **根本原因**: Cloudflare の deployment API が 100% を報告した直後でも、release verifierからの`/health`は旧fingerprintを返し、60秒後には新fingerprintへ安定した。`/health`は`cache-control: no-store`を返し、unique queryでも再現したが、Cloudflare内部の正確な配信経路は観測できないため「全edge PoPの伝播状態」までは断定しない。このrepositoryにはrelease直後の観測差を扱う仕組みが無く、`scripts/check-production-health.mjs`の`validateBridge()` (毎時`worker-health.yml`が実行) と`scripts/run-publisher.ts`の`verifyBridgePublisherContract()` (毎時Publisher runのprecondition) はどちらもimmediateかつfail-closedな単発チェックのみだった。これは正しい設計 (R-027のrecurring checkはstale fingerprintを無期限に許容してはならない) だが、deploy直後に人間が同じsingle-shot確認を手動で行うと、短い観測差を「bundleが壊れている」と誤解しかねない。
- **対策**: `scripts/verify-worker-deploy.mjs` を新設し、`pollForFingerprint()` (fetch/sleep/clock を injectable にした pure helper) で bounded polling を実装した。既定は timeout 120s、interval 5s、requiredConsecutive 3 で、1 回の一致だけでは成功と判定しない (flapping 対策)。timeout 経過後も一致しなければ fail-closed で exit 1 にする (R-027 の意味論は維持)。各 attempt (fetch + body 読み取り) は `AbortController` と、残り予算だけを使う `setTimeoutImpl` (既定 `setTimeout`、test では injectable) への race で個別に bound し、attempt 開始前にも deadline を確認して残り予算を超えて sleep しないようにした (独立 review が「`timeoutMs` は attempt 完了後にしか確認されず、fetch/body 読み取りが hang すると無期限に block し得る」という gap を merge 前に検出し、修正した)。root `package.json` は critical path のため npm script alias は追加せず (追加すると fingerprint が drift する)、`node scripts/verify-worker-deploy.mjs` として直接起動する既存 script 群と同じ運用にした。`scripts/git-hooks/pre-push` の `RUN_WORKER_DEPLOY=1` 経路で `wrangler deploy` 成功後に非 blocking で実行するようにした (deploy 自体は既に成功しているため、propagation 待ちで無関係な push をブロックしない。`SKIP_WORKER_DEPLOY_VERIFY=1` で個別に skip 可能)。`check-production-health.mjs` と `verifyBridgePublisherContract()` は一切変更せず、両方とも immediate/fail-closed のまま維持することをコメントで明記した。
- **教訓**: 「deploy API が100%と報告した」と「release verifierの観測経路が新bundleへ安定収束した」は別の事実であり、後者はbounded pollingと連続成功条件を持つ専用ツールで確認する。ただし単一観測点の連続成功は全edge PoPの収束証明ではない。一方、無関係な原因 (自動化のscheduled precondition) を「propagation待ちだから」と安易にretry対象へ広げるとR-027のfail-closed契約が弱まるため、release時専用の確認とrecurringな自動チェックは明確に別のツール・別のsemanticsとして分離する。**「bounded polling」を名乗るhelperは、attempt間の経過時間だけでなく、各attemptが呼ぶI/O自体 (fetch、body読み取り) も明示的にboundしなければ、実際にはunboundedになり得る**。`await fetchImpl(...)`にtimeoutや`AbortSignal`を渡さないまま「timeoutMsでboundしている」と主張するコメントは、hangする実装やtest doubleに対して簡単に嘘になる。この種のgapはdeterministicな「fetchがresolveもrejectもしない」regression test (injectable timer/clockでreal waitなし) がないと検出しにくいため、bounded-retry helperには必ずそのようなテストを対にする。

### LL-408: 一時git repoへspawnするtestは親processのGIT_DIR等を明示的にcleanしないと、git hook内実行時にだけ落ちる
- **事象**: `worker/`差分判定のfail-closed修正を検証するintegration testが、`npx vitest run`単体実行や繰り返し再実行 (8回超) では毎回PASSしたが、実際の`git push`実行 (pre-pushフックが`npm test`を呼ぶ経路) の中でだけ1回failした。失敗内容は「genuineなworker/差分があるrangeなのに『差分なし』と判定された」という、テストの主張と正反対の結果だった。
- **根本原因**: git hook (pre-push/pre-commit等) はその子processへ`GIT_DIR`等のgit-repository-pinning環境変数を設定する場合があり、test内で`spawnSync("git", ...)`する際に`env`を明示cleanせず`process.env`をそのまま継承すると、その`GIT_DIR`が意図した一時repoではなく**外側の実repoの`.git`**を指してしまう。結果、一時repo用に作ったcommit・rangeが実repoの`.git`へ誤って解決され、`git diff --quiet`が想定と異なる結果を返す。通常のterminalから`npx vitest run`する分には`GIT_DIR`は未設定なので再現せず、`git push`のpre-pushフック経由で`npm test`を実行したときにだけ発現する、環境依存の低頻度flakeに見えた。`GIT_DIR="$(pwd)/.git" npx vitest run ...`で環境変数を強制注入すると、同じ失敗が確実に再現した。
- **対策**: `spawnSync`で一時repoへgitコマンドを実行する全箇所 (repo初期化・commit・rev-parseのrun helper、bash snippetを実行するrunner) を、`GIT_`prefixの環境変数を全て削除した`env`で起動するよう統一した (`tests/protected-branch-guard.test.ts`が既に持っていた`cleanEnv()`と同じ考えを、prefix一致で汎用化した`cleanGitEnv()`として実装)。`GIT_DIR`/`GIT_WORK_TREE`/`GIT_INDEX_FILE`を明示的に注入して再現し、修正後は再現しないことを確認した。
- **教訓**: 一時git repoへ`spawnSync`でgitコマンドを実行するtestは、親processの環境をそのまま継承しない。特にgit hook自身やCIのgit操作から呼ばれるtest runでは`GIT_DIR`/`GIT_WORK_TREE`/`GIT_INDEX_FILE`等が設定され得るため、一時repoを対象にする全てのgit subprocess呼び出しでこれらを明示的に取り除く。通常のterminal実行だけでPASSを確認しても、hookから呼ばれる経路での再現性は別途確認しないと見逃す。同種のtest helperをrepository内に複数作る場合は既存の`cleanEnv()`を横展開し、独自に環境継承する新しいhelperを増やさない。

### LL-409: child sessionからGit作業を引き取る前にprocess停止を実測する
- **事象**: child sessionへ停止指示を送り親がintegrationを引き取った後も、childのCopilot processが処理を継続していた。親がfollow-up commitを作成・pushした約1分後、childがbranchを以前のcommitへresetし、remote refとlocal branchを巻き戻して親のcommitをPR headから外した。
- **根本原因**: cross-sessionの停止messageは非同期であり、送信成功やsessionのidle表示をfilesystem/Git mutation停止の証拠として扱った。child worktreeのprocessとGit reflogを確認せず、同じbranchへ親とchildが時間差でmutationした。
- **対策**: childからGit作業を引き取る場合、停止指示後に対象session processが終了したこと、worktree statusとHEADが一定時間安定していることを実測してから親がmutationする。未commit成果を保持する必要がありarchiveできない場合は、対象child processのPIDを確認して停止し、worktreeを残す。親のcommit/push後もremote refとreflogを再確認する。
- **教訓**: sessionが別worktreeでもbranch/indexの所有権はmessageだけでは移らない。handoffは「指示した」ではなく「旧owner processが停止し、Git stateが安定した」時点で成立する。parent takeover、child resume、automationを同じbranch上で重ねず、ownerを1つに固定する。

### LL-410: 静的buildが知り得ないruntime bindingは同一origin read-onlyのbooleanだけendpointで確認する (LL-100再発込み)
- **事象**: 匿名いいねが`PUBLIC_TURNSTILE_SITE_KEY`未設定で完全に隠れて見えるproduction監査に対し、D1 binding・HMAC secret・Turnstile secretの各readinessを値を一切出さずtruthfulに確認する手段がなかった。実装中、新設した`.reaction-config-flag-list`にも`display:flex`を付けたため、LL-100と全く同型の「authorのclass display指定がUAの`[hidden]{display:none}`を打ち消し、`hidden`属性のtoggleが効かない」バグを自分で再発させ、Playwright E2Eで検出した。
- **根本原因**: Astro `output:"static"`はbuild時にCloudflare Pages Functionのruntime binding (D1・secret) へ一切アクセスできず、`data/index.json`由来の`/metrics.json`もharness/worker生成物なのでPages Functionのbindingを知り得ない。この2つのartifactのどちらにも「configured」を書けないため、read-onlyかつboolean-onlyの新しいsame-origin runtime endpoint (`GET /api/reactions/config`) を追加する必要があった。加えて、`hidden`属性でtoggleするlistへ新規にauthor CSSで`display`を指定する際、LL-100を読んでいたのに同じ罠を踏んだ。
- **対策**: `web/functions/_shared/reactions.ts`に`evaluateReactionConfig(env)`(D1 binding・HMAC secret 32文字閾値・Turnstile secret・public site keyのboolean 4つ + aggregate `configured`のみを返し、値は絶対に返さない)と`handleGetReactionConfig`を追加し、静的route `web/functions/api/reactions/config.ts`をmount。HMAC 32文字閾値は`requireSecret`と共有の`hasMinLength()`へ集約し、fail-closed gateとconfig health checkの判定基準を単一化した。Status側は`web/src/lib/reaction-config-health.ts`(pure derivation、tone は`ok`/`neutral`のみでERR/WARNは絶対に使わない — 安全にdegradeする任意機能だから)と`web/src/lib/reaction-config-client.ts`(6秒timeout・malformed payload guard・例外を投げないbounded fetch)を新設し、SSRは neutral な「確認中」を出し、progressive enhancementで「設定済み(ok)」「未設定(neutral、欠落項目をJA/ENで列挙)」「確認できません(neutral、未設定とは別文言)」の3状態へ解決する。`.reaction-config-flag-list[hidden]{display:none}`を明示追加してLL-100の再発を修正した。
- **教訓**: 静的siteのbuild-time artifactが原理的に知り得ないruntime-onlyな設定状態 (D1/secret bindingなど) は、値を返さないboolean-onlyのread-only same-origin endpointをprogressive enhancementで読む以外に真実を得る方法がない。表示は「未設定」と「確認不能 (endpoint未到達)」を必ず区別し、安全にdegradeする任意機能はabsence/未到達をERR/WARNにしない。`hidden`属性でtoggleする要素へ新しくauthor CSSの`display`ルールを書くときは、既存LLを読んでいても機械的にLL-100の罠を踏み得るため、`[hidden]{display:none}`の明示ペアを追加した直後に実ブラウザ (Playwrightの`toBeHidden()`) で検証する。fail-closedな閾値判定 (HMAC 32文字など) はgateとhealth checkの両方で同じhelperを共有し、判定基準がdriftしないようにする。

### LL-411: 本文 retention は boolean gate だけでは無限に増える。実バイト予算の別レイヤーを priority tier で決定論的に enforcement する
- **事象**: scheduled Publisher run が `data/bodies.json` 10,127,308 bytes で `tests/data-schema.test.ts` の 10MB hard ceiling を超えて失敗した。既に main の committed artifact も 9,942,329 bytes / 1,436 件で、次の1回のrunで確実に超過する予測可能な故障だった。
- **根本原因**: `isBodyRetentionEligible()` (LL-115) は `evergreen === true` または `importance >= 2` の entry を **age を一切見ずに永久に retention 対象**とする boolean gate であり、importance==1 の entry だけが「直近 `BODY_RETENTION_DAYS` 日」で足切りされる。実データでは importance==2 (非 evergreen) が 912/1,436 件 (63%、6.19MB) を占め、live index に残り続ける限り (categoryの半減期次第で数ヶ月) 際限なく蓄積する。retention gate は「この entry は body を持ってよいか」の判定であり、「payload 全体の総 byte 数」という概念を一切持たない。加えて body Queue の enqueue 候補選定 (`selectBodyJobBatch`/`needsBody`) も同じ boolean gate だけを見ており、budget で prune された entry を次 run で無限に再生成→再 prune するwasteループ、および Web 側の「queued (生成待ち)」表示が never resolve する false promise の温床になり得た。
- **対策**: retention boolean gate (`isBodyRetentionEligible`) とは**別レイヤー**として `worker/src/bodies-budget.ts` を新設した。`bodyBudgetPriorityRank()` は evergreen (rank 0、優先度最上位=最後まで残す) → importance>=3 (rank 1) → importance==2 (rank 2) → importance==1 (rank 3、最優先で prune) の優先順位を定義し、`bodyBudgetPruneOrder()` は同 tier 内を古い順 (publishedAt、無ければ collectedAt にフォールバック)、同時刻は id 昇順で決定論的にタイブレークする完全な prune 順序を返す (evergreen も含む全 tier — 「最後まで残す」の意味は次の follow-up 参照)。`serializedByteLength()` は `TextEncoder` で実 JSON (envelope・改行・multibyte UTF-8 込み) を正確に計測し、`enforceBodiesBudget()` はこの決定論的順序に対して **exact な minimal-removal 二分探索** (概算をせず、都度 `serializeBodies()` の実出力を測る) で target 以下になるまで prune する。target は既存の 10MB hard ceiling (`tests/data-schema.test.ts`、変更しない) より十分小さい `DEFAULT_BODY_BUDGET_TARGET_BYTES = 9,000,000` とし、hard ceiling までの 1,000,000 bytes (実測の1回あたり成長量 ~185,000 bytes の何倍もの余裕) を安全マージンにした。Publisher runtime (`worker/src/index.ts` の `runBodyPipeline`、merge 直後・commit 直前に enforcement) と migration (`scripts/clean-source-noise.mjs`) は同じ `enforceBodiesBudget()` を呼び、health へ `bodyBudgetTargetBytes` / `bodyBudgetBytes` / `bodyBudgetPruned` / `bodyBudgetEvictedIds` を追加した。budget で prune された id は次 run の health から読み戻し (`bodyMergePendingIds` と同じ one-hop lookback パターン)、`selectBodyPipelineJobs()` の新しい `opts.excludeBudgetEvictedIds` (既存呼び出しには影響しない optional field) で候補選定から除外し、waste ループと Web の false "queued" promise を防ぐ。実データで migration (`--dry-run` → `--apply`) を実行し、evergreen/importance>=2 は 1 件も prune されず importance==1 (直近) の最古 131 件だけが prune されたこと、live entries の ID・URL・summary は 1,822 件全て無変更であること、prune された entry は依然として live index に残り要約と原文リンクが表示されること、2 回目の `--apply` が no-op (idempotent) であることを確認した。
- **教訓**: 「retention の boolean gate」と「total payload の byte budget」は別々の概念であり、boolean gate だけでは importance>=2 のように**age を見ない tier**が際限なく蓄積し得る。同じ boolean gate の下に priority tier を持つデータでは、budget enforcement は tier + recency の完全な決定論的順序を定義し、近似ではなく実 serialize 結果を都度測って minimal removal を計算する。budget で prune された entry を次回また候補にすると、生成→即時 prune の waste ループと「もうすぐ生成されます」という UI の false promise が生まれるため、prune 結果を次 run の候補選定から除外する one-hop lookback を実装する。hard ceiling (安全網、10MB) と producer target (実際に強制する値、9MB) は別の値にし、target の方だけを厳しくする。migration script と runtime の両方が同じ enforcement 関数を呼ぶことで、boolean gate だけを共有していた過去の実装 (`worker/src/body-queue.ts` の retention helper) と同じ「単一 helper を共有する」原則を budget にも適用する。
- **follow-up (独立レビューで発覚した cross-run state loss バグ)**: 初版の実装は `health.bodyBudgetEvictedIds` へ **その run が新規に prune した id だけ** (`budget.prunedIds`) を書いていた。run1 で id X が prune → `bodyBudgetEvictedIds=[X]`。run2 は `previousBudgetEvictedIds=[X]` を読んで X を候補から除外するため、この run では X に対して何も merge/prune が起きず (X は既に payload から欠落済み) `budget.prunedIds=[]` となり、`bodyBudgetEvictedIds=[]` (空) を書いてしまっていた。run3 は空の list を読んで X を再び候補化 → 再生成 → merge → 即座に再 prune、という**1 run おきの無限 waste ループ**が再発していた ("記録した対象を除外" の実装が「この run の新規イベントだけを記録する」設計になっていたため、除外され続けている間に自分自身の記録を消してしまう典型的な state-loss バグ)。**対策**: `worker/src/bodies-budget.ts` に `carryForwardBudgetEvictedIds(previousIds, entries, bodiesPresent, newlyPrunedIds)` を新設し、前回の evicted id を「(a) 現在も live・retention-eligible、(b) 依然として body 無し、(c) 依然として最低優先度 tier (rank 3、importance==1 非 evergreen)」の 3 条件でフィルタしてから今回の新規 prune と union し、決定論的に `.sort()` した配列を返す。evergreen や importance>=2 へ**昇格**した id は自動的にこのフィルタで解放され再検討対象に戻る (registry/reclassification の変更で復旧できる、という要件を満たす)。`runBodyPipeline` は成功パスだけでなく **disabled / missing-binding / error の全パス**でこの carry-forward を通す (`existingBodiesPresentForFallback` を try ブロックの外で事前計算し、disabled クロージャからも参照できるようにした) — 一時的な mode 切り替え (Queue binding が瞬間的に無い等) でも記憶を失わないようにするため。`scripts/clean-source-noise.mjs` migration も同じ `carryForwardBudgetEvictedIds()` を呼ぶよう統一した。恣意的な 500 件 cap も撤去した (真の集合サイズは retention-eligible かつ body 無しの母集団で自然に有界であり、cap 自体が「まだ正当に除外されているべき id」を落として同じ waste loop を再導入するリスクがあったため)。3 run 分の regression test (`tests/worker-body-pipeline.test.ts`) で run1 prune → run2 は新規 prune 0 件でも telemetry に持ち越し → run3 でも re-enqueue されないことを固定し、disabled/missing-binding mode での持ち越しと、stale (no longer live) / promoted (evergreen・importance 昇格) id の除去も別テストで固定した。
- **教訓 (follow-up)**: 「前回 evict した id を記録して次回除外する」ような cross-run な状態は、**「その run が新たに検出したイベントだけ」を書き込むと、除外され続けている間に自分自身の記録を静かに消す**バグを生みやすい。正しい設計は「前回の記録を today の視点でフィルタ (stale 除去・昇格解放) してから、新規イベントと union して書き戻す」persistent carry-forward であり、これは disabled/error などの一時的なモード遷移でも同じ規則で維持しなければならない (でなければ transient failure が silent な記憶喪失を引き起こす)。恣意的な件数 cap は、対象集合が構造的に有界なら不要であり、有界性の根拠 (今回は「retention-eligible かつ body 無しの母集団」) を明示した上で cap を撤去する方が、cap 自体が再発防止機構を壊すリスクを避けられる。同種の「前回状態を1回だけ記録する」機構 (`bodyMergePendingIds` など) を新設・変更する際は、同じ cross-run persistence の観点で見直す。
- **follow-up 2 (独立レビューで発覚した evergreen 無制限バグ)**: round 2 までの実装は evergreen を `bodyBudgetPruneOrder()` の候補から完全に除外し、`enforceBodiesBudget()` は「evergreen だけで target を超える場合は変更せず、実バイト数をそのまま報告する (best effort)」を正式に許容していた。R-012/R-022 は evergreen の addressability と優先度を要求するが、budget を無期限に超過し続けてよいとは要求していない。evergreen コーパスが (収集ソース追加等で) 育ち続ければ、この「root fix」自体がいずれ元の Publisher hard-ceiling 故障を再現する — 「別レイヤーの byte budget を作った」という対策が、最終的に無制限になり得るのでは意味がない。**対策**: `bodyBudgetPruneOrder()` から evergreen の除外フィルタを削除し、候補プールに含めた。比較関数は `rank` の**降順**でソートするため、rank 0 (evergreen、最も小さい数値) は追加のロジックなしで自然に配列の**最後**に並ぶ (=最後に prune される)。`enforceBodiesBudget()` の「evergreen しか無く prune 対象が空」という早期 return 分岐は、evergreen が候補プールに含まれるようになったことで到達不能なコードになったため削除した — 存在する全ての id は必ず `orphanIds` か `known` のどちらかに入り、`bodyBudgetPruneOrder()` はもう `known` から何も取り除かないため、`pruneOrder.length` は常に `presentIds.length` と一致する (0 件チェックは既に上流で行っている)。二分探索のロジック自体は変更していないが、これにより「record が 1 件でも存在する限り `bytes <= targetBytes` を保証する」という不変条件が正しく成立するようになった (全 id を含む envelope のみの JSON はどんな現実的な正の target よりも小さいため、探索の上限は常に有効)。「保護」の意味を「絶対に prune しない」から「最後に prune される」へ全 docstring・R-012・README で明記し直した。**同時に発覚した二次的な問題**: round 2 の `carryForwardBudgetEvictedIds()` は「現在の rank が固定値 3 でなければ解放する」という判定だったが、evergreen が prunable tier になった以上これは誤りだった — evergreen として evict された id は rank が常に 0 (`!== 3`) なので、この固定判定だと**毎回無条件に解放されてしまい**、evergreen tier に対して全く同じ waste loop を再導入してしまう。**対策**: 判定を「現在 real body を持つ entry の中で最も悪い (数値が最大の) rank」= `worstSurvivingRank` を動的に計算し、除外対象 id の現在の rank が `worstSurvivingRank` より**厳密に良い (数値が小さい)** 場合にのみ解放するよう変更した。これは「今回の budget が実際にどこまで妥協を許しているか」という観測済みの事実を基準にするため、evergreen-eviction (worstSurvivingRank=0 のとき、rank 0 の evergreen-evicted id は `0 < 0` が成立しないため解放されない)、importance 1→2/3 昇格 (現在生き残っている中で最悪の tier より rank が良くなれば正しく解放される)、tie (worst survivor と同じ rank なら保守的に除外継続) の全シナリオを、id ごとの履歴的な rank 記録を持たずに正しく一般化する。既存の promotion-recovery テストは空の `bodiesPresent` を使っていたため (`worstSurvivingRank=-1` となり、いかなる rank でも解放条件 `rank < -1` を満たせず壊れていた) 現実的な survivor context を持つよう書き直し、evergreen-only-survives での非解放と rank tie での非解放を検証する新規テストを追加した。`tests/worker-bodies-budget.test.ts` の「evergreen だけで target を超える場合は変更せず」テストは削除し、代わりに evergreen が必要最小限だけ (最古から) last-resort として prune されることを証明する新規テストへ置き換えた。`tests/data-schema.test.ts` の persistence-contract テストも固定 rank 3 チェックから、実データ (`data/index.json` + `data/bodies.json`) から `worstSurvivingRank` を再計算して照合する検証へ更新した。実データでは evergreen コーパスが現状 ~1.2MB (target 9MB に対し十分小さい) のため、この回の migration re-run は no-op であることを確認した (idempotent, zero content diff)。
...
- **LL-412**: 広い feed を狭い taxonomy に収集する source filter では、本文 snippet に偶然含まれる語を、その taxonomy の主題を示す根拠として扱わない。今回の Zenn AI feed は、業務向け Word レポート生成デモの「ローカルのプロジェクトフォルダ」という本文語だけで `local-llm` に採用され、scheduled Publisher のデータ品質 gate を壊した。Local LLM のように題名から明確な主題証拠を要求できる lane は `keywordFilterScope: "title"` にし、**bare な `ローカル` は題名にあっても主題証拠として使わない**。一方、token boundaryにより`gemma`が`DiffusionGemma`へ一致しないことや、VRAM容量がlocal inferenceの明確な実行制約であることを無視してはならない。`DiffusionGemma`、`LLM` / `SLM`、`VRAM`、`ローカルLLM` / `ローカルAI`、`Ollama`、明示model/runtimeを列挙し、actual corpusのkeep fixtureと、RDS/local-PC/Kaggle/bookmarkなど偶発語だけのdrop fixtureを同時に置く。registry policyを変えたらfresh collectだけで終わらせず、shared filterを使う`clean-source-noise`のdry-runでlive/archiveの影響を定量化し、必要なartifact migrationを同じ変更単位でatomicに適用する。生成JSONの手編集で個別例外を作らない。fingerprintを変えるreleaseのdeploy handoffはR-027を弱めず、PR-head Queue consumerを先にdeployして旧consumerをdrainし、merge後に旧harnessのmarker mismatch（またはR-027の代替安全条件）を観測し、明示承認後にbridge/harnessをdeployする。post-mergeのharness deployだけで完了とは扱わない。
...
### LL-414: crawl discovery は仕様書や配信側の robots 補完ではなく build artifact と route parity で保証する
- **事象**: 本番は Cloudflare managed content signal 付きの `/robots.txt` を 200 で返していたが Sitemap directive が無く、`/sitemap.xml` と `/sitemap-index.xml` は branded 404 を返していた。一方、計画仕様には `@astrojs/sitemap` で自動生成すると記載されていたが、実 dependency、Astro integration、endpoint、build test は存在しなかった。
- **根本原因**: robots.txt が配信されていることと、検索 crawler が canonical route を発見できることを同じ完成条件として扱い、計画文書を build artifact の証拠にしていた。また live、warm、cold、低頻度tag、各paginationの生成条件が複数の `getStaticPaths()` に分散しているのに、crawl対象との parity を検証するcontractが無かった。
- **対策**: web自己完結の `lib/sitemap.ts` が実際の `data.ts` / `archive.ts` の route helperと定数を再利用し、live+warm detail、生成対象tag、カテゴリ・Timeline pagination、Archive月を列挙する。serializerはcanonical origin、query/hash/external排除、XML escape、unique化、50,000 URL / uncompressed 50 MBをbuild import時にfail-closedで検証し、truthfulな更新時刻を証明できないため`lastmod`は省略する。`robots.txt.ts`も`SITE_URL`を参照してSitemap directiveを出し、Cloudflare managed content signal自体はsourceから推測して複製・削除しない。unitで全route family parityとlimitを、built previewのPublisher E2Eで200/content-type/既存404を固定する。
- **教訓**: sitemap/robots/feedのようなcrawler向け公開面は「設定予定」やedgeの自動応答を実装証拠にせず、production build成果物のstatus・content-type・本文と未知routeの404を同時に検証する。高cardinality SSGでは別の巨大route registryを作らず、実 `getStaticPaths()` と同じ集合・閾値helperを共有し、addressability tierや生成閾値が変わったときの差分をroute parity testでfail-closedにする。
- **follow-up (独立レビューで発覚した cold detail 混入)**: 初版は sitemap と記事 `getStaticPaths()` の双方が `ALL_ENTRIES` を無条件に addressable とみなし、live index に残る `archiveTier:"cold"` 132件へ個別 detail route と sitemap URL を生成していた。さらに parity test も同じ無条件の `ALL_ENTRIES` から期待値を作ったため、実装と期待値が同じ誤りを共有してPASSした。JSONをimportしない `detail-addressability.ts` に untiered live / hot / warm を許可し cold / dropped を拒否するpredicateとdedupe collectorを抽出し、記事route、sitemap、indexed detail tag recoveryが同じhelperを同じ入力集合へ適用するよう統一した。testはpure synthetic tier表に加え、実artifactのcold全IDがsitemapに無いこと、代表hotとarchive-only warmがあること、built previewでhot/warm=200・cold=404を独立に確認する。
- **教訓 (follow-up)**: route parity testの期待集合をproduction codeと同じ未検証collectionから作ると、collection自体の過包含を検出できない。tier/visibility contractは、(1) JSON-free pure policyのsynthetic境界test、(2) 実artifactで禁止tierを全件negative assertion、(3) 代表許可tierのpositive assertion、(4) built HTTP status、の独立した4層で固定する。`ALL_ENTRIES`のような名前を「全件addressable」と読み替えず、route生成前に明示policyを通す。
- **follow-up (detail 404 を消しても inbound link が残った)**: cold detail route を削除した後も、一覧 component と記事内の関連・前後 link が `entryHref()` や `/e/${id}/` を無条件に描画し、生成済み `/page/53/` から cold ID `11f7a942f92bd4c4` の 404 へ到達できた。また unit の sitemap 期待値は各 `getStaticPaths()` の pagination 式を複製しており、同じ式の drift を実 build に対して検出していなかった。
- **対策 / 教訓 (follow-up 2)**: JSON-free `entry-destination.ts` が addressability policy を再利用し、untiered / hot / warm を内部 detail、cold / dropped を canonical source として全 reader-facing entry link に適用する。pagination と canonical path は `route-inventory.ts` を sitemap と Astro route で共有する。さらに production build 後に sitemap path と実 `dist/**/index.html` の canonical non-redirect inventory を双方向比較し、redirect-only sitemap 混入と全生成 HTML の内部 detail link が実在する route を指すことを固定する。shared helper の unit parity だけを実 artifact parity の代用にせず、browser で cold listing card の source destination と代表 hot / warm の内部 destination も確認する。
- **follow-up (archive-only ID と new-tab intent の見落とし)**: 初版の build validator は禁止IDを live `data/index.json` の cold / dropped だけから作ったため、月別 Archive にしかない cold ID を検査できなかった。また cold link を source URL へ切り替えて `target="_blank"` にしたが、リンク先が新規tabで開くことを icon と accessible name のどちらにも一貫して示していなかった。
- **対策 / 教訓 (follow-up 3)**: validator は tier collection を再構築せず、canonical HTML 内の同一origin `/e/{id}/` hrefを正規化し、生成済み canonical detail route 集合に存在することを一般不変条件として検証する。fixtureはarchive-onlyの存在しないIDを必ず失敗させ、hot/warm実在routeを許可する。新規tabのentry destinationは共通 `ExternalLinkHint` を使い、visibleな装飾icon `↗` (`aria-hidden`) と、現在のJA/EN表示に対応するvisually-hidden補足をリンクのaccessible nameへ含める。internal linkにはhintを出さず、desktop/mobileと両言語をbrowserのaccessibility treeで検証する。
- **follow-up (相対 href の base と external rel の drift)**: validator が全 href を origin root (`/`) 基準で解決していたため、nested canonical HTML の `../` / `./` link は browser と異なる path として評価された。また共通 destination と EntryCard が `rel` literal を別々に持ち、既存の `noreferrer` を落とした。
- **対策 / 教訓 (follow-up 4)**: build validator は各 `dist/**/index.html` に対応する trailing-slash canonical route を document base URL として `new URL(href, documentUrl)` で解決し、nested parent/dot relative、root-relative、absolute same-origin、query/hash-only、HTML entity を独立fixtureで固定する。external entry の rel は `EXTERNAL_ENTRY_REL = "noopener noreferrer nofollow"` を唯一の値とし、EntryCard override もそれを消費する。internal hot/warm は target/rel属性を持たないことをunit/browser双方で確認し、URL resolverやsecurity属性の literal をcallsiteへ複製しない。
- **follow-up (regex は HTML parser ではない)**: quote 前提の href regex は unquoted 属性と decimal/hex numeric entity を見落とし、script/style/comment 内の `href=...` 文字列を実 link と誤認した。URL の base だけ browser と揃えても、attribute tokenization と character-reference decoding が browser と異なれば build invariant は false negative / false positive の両方を持つ。
- **対策 / 教訓 (follow-up 5)**: web lock に既存の `parse5@7.3.0` を明示 devDependency として宣言し、validator は各 HTML を標準 parser で document tree にして、実 element の decoded `href` 属性だけを巡回する。HTML/SVG 等の foreign node でも `attrs` / `childNodes` の存在を型形状で確認し、script/style/comment text は自然に除外する。quoted/unquoted、named/decimal/hex entity、malformed recovery、relative/absolute/protocol-relative/external、fake textを独立fixtureで固定し、HTML構文をregexで再実装しない。
...
- **follow-up**: macOSで`system-ui`を先頭にしたstackは全122件E2Eを通ったが、Linux CIでは721pxのHeader/Top-3 overflow、390pxの`Agent Frameworks` clip、Featuredのfirst-view 8px超過、320pxのcompact tagline 2行化が再現した。OS既定fontのmetricsを同一と仮定したことが原因だった。LatinはArial/Liberation互換を先頭に固定し、日本語はHiragino/Yu Gothic/Meiryo/system fallbackへ流すstackへ変更した。Linuxで残ったfirst-view差はtouch targetや本文を削らず、mobile Hero paddingの既存下限を維持しながらpriority見出しの非操作paddingを合計8px圧縮し、compact taglineは意味を保った短文へ変更してnowrapで隠さず1行へ収めた。外部font削減はnetwork gateだけで完了にせず、Linux CIの境界幅・first-view・label clipを必須とする。

### LL-416: privacy controlは目的別dependencyと実際のtransport・retentionを分離する
- **事象**: current-browser匿名いいね削除が、D1とHMACだけで実行できる処理なのにTurnstile secretまで要求し、mutation検証が未設定だと利用者自身の削除controlも停止していた。privacy copyも表示言語と検索がshareable URLへ入ること、D1のactive identity・票・rate-limit行に自動削除期限がないことを明記せず、localStorageとbrowser内検索だけを説明していた。
- **根本原因**: 匿名いいねserviceの「mutationに必要な全dependency」をidentity cleanupにも一括適用し、機能別のpurpose limitationをserver gateへ反映していなかった。client側ではbrowser storage、URL query、delivery request、第三者request、server-side retentionを別のdata flowとして棚卸しせず、局所的な実装説明をprivacy disclosureへ流用していた。
- **対策**: reaction serviceをD1+HMACのidentity storeと、そこへTurnstileを加えるmutation serviceへ分離し、same-origin DELETEはcookie不在ならbinding不要で冪等、cookieありならD1+HMACだけでactive identity・票・rate-limit行を削除するようにした。privacy pageはversioned explicit opt-in、production custom-domain gate、`?lang`/`?q`/`?tag`/`?entry`/`?category`/`?ids`のdelivery visibility、外部media、localStorage、Cookie、D1の自動期限なし、第三者policyをJA/ENで明示し、保存不能・削除失敗・44px/focus/active-languageを実ブラウザで検証する。
- **教訓**: privacy controlは「同じfeatureに属する」だけでdependencyを共有させず、collect、verify、store、deleteの各目的に必要な最小dependencyへ分解する。privacy disclosureは「localに保存した値」だけでなく、URLへ反映されるstate、通常の配信request、外部resource request、自動削除の有無をdata flowごとに確認し、未観測や未実装の保持期限を推測で約束しない。削除controlはoptionalなmutation verificationが停止していても、削除に必要なstorageとidentity keyが利用可能なら実行できるようにする。

### LL-417: identity削除と並行POST/PUTはactive identityへfail-closedに接続する
- **事象**: 匿名いいねDELETEが既存の票とrate-limit行を削除しても、Turnstile検証済みの並行PUTがDELETE後に`setLiked()`へ到達すると、失効済みCookieのhashで票とrate-limit行を再生成できた。初期修正後も、同時進行のidentity POSTがDELETE後にactive row不在を検出すると、同じrequest内で別UUIDとidentityを即時再生成できた。`article_likes`のprimary keyは`(article_id, voter_hash)`順で、current-browser削除の`WHERE voter_hash = ?`もfull-table scanだった。
- **根本原因**: browser cookieのhashが現在もactiveかをserver-side storageで表現せず、rate-limit更新、票変更、identity作成、identity削除を別々のD1 transactionとsuccess-shaped fallbackへ分けていた。削除APIの成功と、進行中POST/PUTのcommit可否を同じidentity stateへfail-closedに接続していなかった。
- **対策**: `reaction_voters` active identity tableと`article_likes(voter_hash)` indexをmigrationで追加し、既存票/rate-limit hashをbackfillする。identity bootstrapはcookieなしのrequestだけが新しいidentityを作成する。active rowが無い既存Cookieを受けたPOSTは`409 identity_required`とcookie失効を返し、同じrequestでは再生成しない。PUTはactive identity確認、rate-limit更新、票変更、count読込を1つのD1 batch transactionへまとめ、DELETEはactive identity、全票、rate-limit行を別の1 transactionで削除する。DELETEが先行した並行POST/PUTはともに`409 identity_required`となり、dataを再生成しないrace testと実SQLite query planで固定する。
- **教訓**: user-triggered deletionは「現在あるrowを消した」で完了にせず、in-flight writeが削除後に再生成できないcommit gateを持たせる。匿名cookieのようにclient側だけでidentityを表す設計では、server側に最小のactive identity stateを置き、mutationとdeleteをそのstateへ条件付ける。削除queryは実行計画まで確認し、privacy control自体がunbounded scanやabuse surfaceにならないreverse indexを用意する。

### LL-418: consent撤回は全tabの実行済みscriptと永続的なreturn pathまで閉じる
- **事象**: AdSenseを許可後に別tabで広告を拒否しても、storage listenerはstate表示だけを更新し、既に実行済みのAdSenseを止めなかった。同一tabのreloadはfocusを`body`へ落とした。さらにmobileではpromptとfooterが非表示になった後、MenuにPrivacyが無く、広告選択や削除controlへ戻れなかった。短いviewportのfixed promptと展開した削除confirmationも主contentやtabbarへ重なった。
- **根本原因**: consent stateをlocalStorage recordとDOM表示だけの問題として扱い、実行済み第三者scriptのunload、cross-tab propagation、reload後focus、決定後の再到達性、fixed UIとのgeometryを同じstate transitionへ含めていなかった。
...
### LL-426: 見た目で覆われたcontrolはTab・pointer・AXから自動的には消えない
- **事象**: 390x844と375x667で固定Searchの閉じるボタンがJA/EN言語切替と1,848平方CSS px重なり、言語ボタンの中心hit targetは閉じるボタンになった。一方、背面のJA/ENはvisible判定、Tab順、accessibility treeに残っていた。
- **根本原因**: 固定overlayをviewport端へ配置しながら、同じHeader内の既存controlとの矩形交差とhit-testingを検証していなかった。z-indexで見えなくなっても、背面controlのfocusabilityとaccessibility semanticsは変わらない。
...
### LL-451: evergreen sourceとKnowledge掲載適格性を同一視しない
- **事象**: AWS ML BlogとGoogle Cloud Blogをsource単位でevergreenにした結果、実践how-toに混じってモデル提供開始、一般提供、preview、月次告知、award、event登録もKnowledge HTML/RSSへ恒久掲載された。
- **根本原因**: retentionのsource-level `evergreen`をentry単位のKnowledge掲載許可として流用し、raw title/snippetから時点依存の告知だけを除く境界が無かった。
...
### LL-467: GitHub schedule の欠落は起動時刻の分布で原因を判定してから直す

- **事象**: Publisher の `0 * * * *` が 1 日 24 回中 5〜9 回しか起動せず、Worker Health が `publisher run is stale` で失敗し続けた。「毎時 0 分が混雑している」と判断して `13,43 * * * *` に変更し 1 回あたりの上限を半減したが、変更後 9 時間で定期実行は 1 回だけで、処理量だけが落ちたため取り消した。
...
- **対策**: 対話パネルをinline-size containerとして扱い、内容幅が930px以下では画像と見出しの次の行へ説明を2列分の全幅で置く。十分な幅のdesktopでは既存の右列配置を保つ。AIバッジと文を小さな2列Gridで揃え、320〜1280pxのJA/ENで説明幅、バッジとの間隔、語句の行位置、横overflowをDOM寸法で検証する。
- **教訓**: 語中分断を`nowrap`で全段落へ強制したり短縮文だけで隠したりする前に、その文へ配分されたGrid trackの幅と隣接画像の占有を実測する。情報の階層は保ったまま、利用可能なコンポーネント幅に応じて説明の所有行を変え、バッジと本文は折返し後の開始位置まで揃える。
