...
- サイト監査 第 3 弾(表示の仕上げ)。各ページのヒーロー統計に残っていた内部用語(live index / taxonomy / artifact / snapshot / Queue / run)を読者向けの補足に置き換え、英語表示でも日本語のまま残っていた箇所(カテゴリ一覧のグループ説明・「最終更新」・「エントリなし」、タグページの説明と空状態、右レールの「ソースの稼働状況」「最新記事」)を日英切替対応にしました。
- 文字主体の OG 画像(Qiita / Zenn / GitHub 等)がカードとスポットライトのサムネイルで断片的に切り取られていた問題に対応し、画像全体を収めつつ余白を同じ画像のぼかしで埋める表示に変更しました。
- 月別アーカイブの「Day index」が先頭 10 日分しか列挙していなかったのを、その月の掲載日すべてに広げました。
- サイト監査 第 2 弾(表示側)。読者に不要な内部情報と紛らわしい表示を整理しました: サムネイル隅の取得元トークン(og / media)を非表示に、/search/ で検索パネルが自動展開して見出しを隠す挙動をやめ(「検索を開く」で開く)、Status の「匿名いいねの設定状態」からシークレット名の一覧を撤去、About の仕組み説明を内部フィールド名・KV・commit 等を使わない読者向けの 4 ステップに書き直し、編集方針の広告記述を About / Privacy と同じ「掲載しています」に統一し見出し横の「59 sources」を削除、記事詳細の重要度が「重要度 重要度 Info」と二重に見える問題と凡例の英語固定を修正、要点を再掲するだけだったプルクォートを廃止、Daily Summary の注記から内部用語(JST ライブ / archive / 保持ポリシー / lanes)を除き 0 件のときは凡例を出さない、ブラウザタブの記事タイトルで出典名が「Anthropic Engineeri…」と語中で切れる問題と日付のみの記事に「00:00 UTC」が付く問題を修正、arXiv のフィルターチップにモバイルでも分野の意味が分かる title を付与。
