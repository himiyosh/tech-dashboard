...
- Full Playwrightは1 workerで直列実行し、同一previewへPublisher E2Eと全UI smokeが同時アクセスしてpeak負荷を増やさないようにしました。pre-pushは生成Home・記事詳細・404・warm/cold・exact tag・nav・PageHeroのE2Eを実行し、全件はPR CIで必須とします。
