...
- **LL-412**: 広い feed を狭い taxonomy に収集する source filter では、本文 snippet に偶然含まれる語を、その taxonomy の主題を示す根拠として扱わない。今回の Zenn AI feed は、業務向け Word レポート生成デモの「ローカルのプロジェクトフォルダ」という本文語だけで `local-llm` に採用され、scheduled Publisher のデータ品質 gate を壊した。Local LLM のように題名から明確な主題証拠を要求できる lane は `keywordFilterScope: "title"` にし、**bare な `ローカル` は題名にあっても主題証拠として使わない**。`ローカルLLM` / `ローカルAI`、`Ollama`、明示model/runtimeなどを列挙し、偶発語だけの記事を drop、subject-specific title signal を持つ近接例を keep する regression test を同時に置く。registry policy を変えたら fresh collect だけで終わらせず、shared filter を使う `clean-source-noise` の dry-run で live/archive の影響を定量化し、必要な artifact migration を同じ変更単位で atomic に適用する。生成 JSON の手編集で個別例外を作らない。
