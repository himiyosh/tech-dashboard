...
- **対策**: full Playwrightを`workers: 1`へ固定し、Publisher E2Eとsmokeを同じartifact上で直列実行する。timeout、retry、geometry閾値は緩めず、responsive matrixは1 navigation後のviewport変更、複数routeの静的contractはbrowser内fetchへ集約してI/O自体も削減する。pre-pushは生成Home・記事詳細・404・warm/cold・exact tag・nav・PageHeroのE2Eを1 processで実行し、全PlaywrightはPR CIのclean runnerで必須とする。
