...
function deriveSections(paragraphs: string[], _lang: BodyLang): BodySection[] {
  // Legacy plain-text bodies get NO synthesized headings. The 概要/詳細/今後の
  // 展望 labels were derived from paragraph position and a few lexemes, and the
  // site audit found them mislabelling content on most pages (a "詳細" section
  // that only said "原文で確認を推奨", an "outlook" that described the target
  // reader) while giving every article the same three-entry table of contents.
  // Headings and the TOC now come only from authored "## " lines.
