...
- **事象**: 62 filesを含むmerge commitのpushで、pre-push hookは`git diff --name-only`から17件のWeb影響fileを得られる状態なのに`web/への影響なし`としてPublisher E2Eをskipした。同じSHA rangeをpush後に直接検査すると17件が一致した。後日の別レビューで、同じhook内の`worker/`差分判定 (`git diff --name-only "$range" -- worker/ | grep -q .`) と、`pre-commit`のstaged TypeScript file判定 (`git diff --cached --name-only | grep -qE '\.(ts|tsx)$'`) にも同型のpipeが残っていることが見つかった。
- **根本原因**: `set -o pipefail`下で`echo "$CHANGED" | grep -qE ...`を使っていた。`grep -q`が先頭matchで終了した後、大きな`echo`がSIGPIPEになるとpipeline全体がnonzeroになり、先頭の`!`が「matchなし」へ反転した。短いfile listではproducerが先に完了して再現しない。同じpipe shape (`git diff ... | grep -q...`) は、fileの種類・判定方法が違うだけで同じhookファイル内、さらに別のhookファイルにも独立に存在し得る。
- **対策**: producer pipeを廃止し、`grep -qE ... <<<"$CHANGED"`で同一process inputを渡す。`worker/`差分判定はfile一覧が不要なため、pipeを完全に避けられる`git diff --quiet "$range" -- worker/`に置き換えた。`pre-commit`のTS file判定は`staged_files=$(git diff --cached --name-only)`でcaptureしてから`grep -qE '\.(ts|tsx)$' <<<"$staged_files"`へ渡す、同じcapture-then-here-stringパターンにした。source contract testでhere-string使用/`git diff --quiet`使用と旧`echo | grep -q`・旧pipe不在を固定し、次のWeb影響pushでPublisher E2Eが実行されることをhook出力で確認する。
- **教訓**: `pipefail`と早期終了するconsumerを組み合わせると、match成功でもproducerのSIGPIPEがpipeline失敗へ見える。判定用の小さな文字列はpipeせずhere-stringまたはfile inputを使い、短いfixtureだけでなく多数fileのmerge rangeでも実行経路を検証する。**1箇所でこのpipe shapeを見つけたら、同じhookファイル内と兄弟hookファイルの両方を`grep`で横断検索し、同型の`git diff ... | grep -q`が他に残っていないか確認する** (whack-a-moleにしない)。単純な「diffが存在するか」の判定には、file一覧を経由せず`git diff --quiet`のようなpipe不要のsubcommandを優先する。
