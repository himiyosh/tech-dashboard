...
- 記事詳細の日本語・英語の AI 解説を左揃えの読書カラムへ整え、過度な字間伸長を解消しました。出典・公開日時・重要度は見出し直下で確認でき、関連トピックは読める表示名と全件開示へ、比較指標と収集記録はキーボードでも開ける記事情報へ整理しました。カテゴリ内での重要度比較も表示中の重要度と同じ尺度に揃え、要約の生成状態と元記事への確認導線は本文近くに残しています。
- develop向けPRとdevelop pushのCIは、同じremote main SHAから取得した生成dataをunit、Web build、E2Eで共有します。mainの36時間鮮度ゲートを維持し、PRが変更した生成dataは上書きせず拒否します。
- 記事末尾のAI対話を、ポコの選ばれた非公開デザイン参照の特徴から描き起こした試作SVGと、TECH Dashboard 独自の「TECHガイド」で見せるようにしました。参照画像そのものはサイトへ配置しません。対話欄では制作過程でなくAI生成と記事の要約・収集情報をもとにした内容であることを簡潔に説明し、狭い幅では絵の下の全幅に配置して日本語の語中分断を防ぎます。スマートフォンでも両方のキャラクターを表示し、対話の読み幅を保ちます。既存の保存済み台本と記事の事実は変更せず、旧配役への明らかな呼びかけのみ表示時に調整します。今後の対話生成も新配役に合わせます。
...
- 2026-09-14 に入れた Publisher の起動時刻変更(毎時 13 分・43 分、1 回あたりの上限半減、収集バッチの 30 分切り替え)を取り消し、変更前の設定(毎時 0 分、上限 35、1 時間ごとのバッチ切り替え)に戻しました。変更後 9 時間で定期実行は 1 回しか起動せず、変更前の 2 週間の起動時刻も 0〜59 分に均等に散っていたことから、原因は毎時 0 分の混雑ではなく GitHub の予約実行そのものの遅延と間引きだと判断しました。上限を半分にしたまま起動回数が増えないと処理量だけが落ちるため、元に戻しています。起動を Cloudflare の定期実行へ移す対応を別途進めます。

## 2026-09-14

### 変更

- Publisher の定期実行を毎時 0 分から、毎時 13 分と 43 分の 1 時間 2 回に変更しました。毎時 0 分は GitHub の予約実行が最も混み合う時刻で、2026-08-30 以降は 1 日 24 回中 5〜9 回しか起動せず、サイトの更新が平均 3 時間おきになっていました。Worker Health の失敗 27 件のうち 18 件はこの間隔超過が原因です。1 回あたりの本文・要約の投入上限と記事本文の取得上限は半分にし、1 時間あたりの処理量を据え置いたうえで、Cloudflare KV の 1 日あたり書き込みが無料枠を超えないようにしました(実行回数 × 上限を 900 件以下に固定するテストを追加)。収集ソースの 6 バッチも 30 分ごとに切り替え、同じ時間帯の 2 回の実行が同じソースを取り直さないようにしています。

### 変更

- 記事詳細の「ソラと博士の読みどころ」から登場人物紹介のカードを削除し、スマートフォンでの読みやすさを直しました。375px 幅の実測で、対話の文字は 13.5px・本文幅 241px(1 行 約 18 文字)と、記事本文の 16.5px・305px より小さく、さらに発言ごとにアバター分だけ左右へ字下げされて視線の起点が動いていました。スマートフォンではアバターを名前の行に移して吹き出しを全幅にし、文字を 15.2px・行送り 1.85 に広げています(1 行 約 22 文字)。デスクトップ表示は従来どおりです。

### 修正

- 要約 Worker のフォールバック末尾に指定していた `claude-sonnet-4.6` が API から提供されなくなっており、gpt-5.6 系が一時的に失敗した実行で打つ手がなくなっていました。要約自体は前段のモデルで生成できていましたが、Worker の死活確認が 1 日 2 回ほど 503 を返し、Worker Health ワークフローが 2026-09-04 以降 108 回中 18 回失敗していました。フォールバックを `claude-sonnet-5` に更新し、コード既定値と許可リスト、ドキュメントの記載も揃えました。あわせて `npm run worker:models` を追加し、設定したモデルが API に存在するかを確認できるようにしています(`COPILOT_PAT` 未設定時はスキップ)。
...
- 収集時に元記事ページ本文を取得して抜粋(`contentSnippet`)にする「記事抜粋レーン」を追加しました。フィードの説明文は 100〜300 字が大半で、抜粋量に比例する本文長バンドが約 330 字で頭打ちになっていたため、記事ページの主要テキストを決定論的に抽出し(`<article>`→`<main>`→本文、ナビ・フッター・スクリプト除去、30 字未満の行を除外)、収集側と本文生成側で共有する 900 字上限に揃えます。毎時 40 件まで(新着優先、その後は既存記事を新しい順に backfill)、試行結果は `excerptOrigin` として記録し失敗を毎時再試行しません。取得失敗時はフィード説明文を維持します。
### 変更

- サイト監査 第 2 弾(表示側)。読者に不要な内部情報と紛らわしい表示を整理しました: サムネイル隅の取得元トークン(og / media)を非表示に、クエリなしで /search/ を開いたときに検索パネルが自動展開して見出しを隠す挙動をやめ(「検索を開く」で開く。?q= やタグからの検索リンクは従来どおり即座に開く)、Status の「匿名いいねの設定状態」からシークレット名の一覧を撤去、About の仕組み説明を内部フィールド名・KV・commit 等を使わない読者向けの 4 ステップに書き直し、編集方針の広告記述を About / Privacy と同じ「掲載しています」に統一し見出し横の「59 sources」を削除、記事詳細の重要度が「重要度 重要度 Info」と二重に見える問題と凡例の英語固定を修正、要点を再掲するだけだったプルクォートを廃止、Daily Summary の注記から内部用語(JST ライブ / archive / 保持ポリシー / lanes)を除き 0 件のときは凡例を出さない、ブラウザタブの記事タイトルで出典名が「Anthropic Engineeri…」と語中で切れる問題と日付のみの記事に「00:00 UTC」が付く問題を修正、arXiv のフィルターチップにモバイルでも分野の意味が分かる title を付与。
- 旧世代の AI 解説本文に自動付与していた見出し「概要 / 詳細 / 背景 / 今後の展望」を廃止しました。段落の位置だけから導いた見出しは内容と一致しないことが多く、全記事が同じ目次になっていたためです。見出しと目次は本文が「## 」で持つ場合のみ表示します。
- サイト監査(UI/UX・内容・内部情報露出)の第 1 弾として、読者に不要な運用情報を全ページから取り除きました。フッターの「batch 2/6 · sources 10/10 · summary 6 · body N pending · harness v0.1 · powered by Astro + Claude Opus」は「正常に更新中 · 更新 2h ago」のような平易な状態表示だけに整理し(詳細は /status に集約)、ヒーローの稼働表示も「index 更新」等の内部用語を排して「更新」に統一。要約未生成カードは「要約を準備中」の一語だけを示し、キュー状態の説明と /status への誘導を外しました。記事詳細の「Queue に投入済み」、カテゴリのトレンド注記(`data/stats.json`・vivid)、アーカイブの hot/warm/cold・snapshot・artifact 説明も読者向けの言葉に置き換えています。
- 日付見出し(TODAY / YESTERDAY / SAT, AUG 29)と 2 ページ目以降の見出し「Entries」を日英切替対応にしました(JA では「今日 / 昨日 / 8月29日(土)」「記事一覧」)。
- モバイルのカード表示を改善しました: 日本語タイトル・要約の不自然な折返し(word-break: keep-all)を解消し要約を 5 行まで表示、重要度ピルが「重要 / 度 / Info」と折れないよう固定、理由ラベルは省略せず次の行に折返し、Status ページの「最新記事」ラベルとカテゴリ一覧の内訳チップの文字欠けを修正。
- 右レールの「Trending topics」から #release / #changelog / #news / #bug-fix などの記事種別タグを除外し、話題のタグだけを表示するようにしました。

### 修正

- 生成コンテンツの品質(サイト監査 第 2 弾)。要約・本文・対話の生成契約と品質ゲートを見直しました: ①日本語記事の見出しは著者の原題をそのまま使い(AI による言い換えを禁止、既存 116 件は原題に戻す)、英語見出しは要約の切り詰めではなく自然な英語ヘッドラインを生成、②本文・対話が「抜粋」「収集元」「既存の要約」「タグ」など生成材料に言及することを禁止し、タグを事実として渡さない、③ソラ×博士の対話は記事ごとに 3 種類の会話の型を使い分け、「簡単に言うと」「それって何が嬉しいの？」等の決まり文句を禁止、④リリースノートの要約は「バグ修正や安定性の改善」だけを不可とし具体的な変更点を必須化、⑤要約に git トレーラー・「Read the full article」・「appeared first on」などのフィード定型文や、ハングル等の他言語スクリプトが混入した場合は不正として再生成、⑥「The post … appeared first on …」だけの抜粋は本文の根拠として認めない。既存データは `scripts/reconcile-content-quality.mts` で整合(原題復元・汚染要約の再生成・根拠不十分/材料言及の本文の再生成)。

### 修正

- タイムライン後方ページなどで cold tier の記事カード(約 215 件)のタイトルが元記事へ直接遷移していた問題を修正しました。cold 記事にも記事詳細ページを生成し(`noindex, follow`・sitemap 非掲載)、タイトルは常に本サイト内へ遷移します。アーカイブ専用の cold 行(route 非生成)はアーカイブ月ページで従来どおり元記事リンクを維持します。要約が存在しない記事(素材不足の RC リリース等)のみ、引き続き元記事へリンクします。

- 毎時 Publisher が 2026-08-30 以降、チャット graft 数を含む `bodyMerged` がルックアップ数の不変条件を超えて全 run 失敗していた問題を修正しました。telemetry に `chatLookupCount` を追加し、不変条件を「bodyMerged ≤ bodyLookupCount + chatLookupCount」に拡張。あわせて chat graft の毎時上限を 20→120 に引き上げ、backfill 済み全記事の対話表示が約 1 日で行き渡ります。
- gpt-5.6 生成の本文が約 330 字と短かった問題に対応しました。原因はモデルではなく、出典抜粋量に比例する長さ制限(捏造抑止)に対して収集抜粋が 280 字に切り詰められていたこと。収集上限を本文生成側の予算と同じ 900 字に引き上げ、豊富なフィードは正当に 540〜900 字の本文を得ます。既存の短文 133 件は削除し(`scripts/prune-short-model-bodies.mts`)、毎時パイプラインが再生成します。

### 変更

- デスクトップ表示(768px 以上)の全体密度を 90% に引き締めました(ユーザーフィードバック「ブラウザ倍率 90% がちょうどいい」)。モバイルはタッチターゲット契約を維持するため 100% のままです。

- 記事カードのタイトルが常に本サイトの記事詳細ページ(要約ページ)へ遷移するようになりました。従来は公開ゲート未承認(hold)の記事だけタイトルが元記事へ直接遷移していましたが、hold 中の記事にも詳細ページを生成し(`noindex, follow`・sitemap 非掲載)、ゲートは検索エンジンへの公開制御(index 可否と sitemap の段階解放)に専念します。要約未生成・cold/dropped の記事は引き続き元記事へリンクします。
- モバイル表示でカード footer の「元記事」「記事詳細」ラベルが「元…」と省略される問題を修正しました。タグが多い場合はリンク pill が潰れず折り返して次の行に配置されます。
