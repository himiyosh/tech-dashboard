...
- モバイルのカテゴリフィルターを、折り返し 7 行(実測約 413px、画面のほぼ半分)から横スクロールする 1 行のレール(約 94px)に変更しました。常時表示・44px の操作面・全 13 カテゴリの切替は維持し、既定でミュートされているカテゴリをレール先頭に固定して、スクロールせずに操作対象が見えるようにしています。ページ自体は横スクロールしません。
