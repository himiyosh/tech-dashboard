...
- production Web buildを1回だけ再利用し、desktop/mobileのHome判断面、検索、RSS/OPML購読発見、実404回復、optionalな要約待ち状態をローカル計測するPlaywrightジャーニーを追加しました。出力はstep条件・操作上限・document navigation列・viewport/scroll・route・失敗観測と情報値の経過msを含む64KiB以下のJSONです。合否は到達性と複雑度だけで判定し、timingはfield dataやCore Web Vitalsではありません。
