...
### LL-417: identity削除と並行mutationはactive identityを含む同一transactionで直列化する
- **事象**: 匿名いいねDELETEが既存の票とrate-limit行を削除しても、Turnstile検証済みの並行PUTがDELETE後に`setLiked()`へ到達すると、失効済みCookieのhashで票とrate-limit行を再生成できた。`article_likes`のprimary keyは`(article_id, voter_hash)`順で、current-browser削除の`WHERE voter_hash = ?`もfull-table scanだった。
- **根本原因**: browser cookieのhashが現在もactiveかをserver-side storageで表現せず、rate-limit更新、票変更、identity削除を別々のD1 transactionへ分けていた。削除APIの成功と、進行中mutationのcommit可否を同じidentity stateへ接続していなかった。
- **対策**: `reaction_voters` active identity tableと`article_likes(voter_hash)` indexをmigrationで追加し、既存票/rate-limit hashをbackfillする。identity bootstrapはactive rowが無い既存Cookieを再利用せず新しいUUIDを発行する。PUTはactive identity確認、rate-limit更新、票変更、count読込を1つのD1 batch transactionへまとめ、DELETEはactive identity、全票、rate-limit行を別の1 transactionで削除する。DELETEが先行した並行PUTは`409 identity_required`となり、dataを再生成しないrace testと実SQLite query planで固定する。
