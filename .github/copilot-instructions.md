...
- **根本原因**: Cloudflare の deployment API が 100% を報告することは control plane 側の rollout 記録が完了したことを意味するだけで、全 edge PoP が新しい isolate を実際に serve し始めたことを保証しない。`/health` は `cache-control: no-store` を返すため CDN cache の問題ではなく、genuine な edge propagation の遅延 (実測で最大 60 秒程度) だった。この repository には release 時の propagation 待ちを扱う仕組みが無く、`scripts/check-production-health.mjs` の `validateBridge()` (毎時 `worker-health.yml` が実行) と `scripts/run-publisher.ts` の `verifyBridgePublisherContract()` (毎時 Publisher run の precondition) はどちらも immediate かつ fail-closed な単発チェックのみだった。これは正しい設計 (R-027 の recurring check は stale fingerprint を無期限に許容してはならない) だが、deploy 直後に人間が同じ single-shot 確認を手動で行うと、正常な propagation window を「bundle が壊れている」と誤解しかねない。
...
- **教訓**: 「deploy API が 100% と報告した」と「全 edge PoP が新 bundle を serve している」は別の事実であり、release 直後の health 確認は bounded polling + consecutive-success requirement を持つ専用ツールで行う。一方、無関係な原因 (自動化の scheduled precondition) を「propagation 待ちだから」と安易に retry 対象へ広げると R-027 の fail-closed 契約が弱まるため、release 時専用の確認と recurring な自動チェックは明確に別のツール・別の semantics として分離する。**「bounded polling」を名乗る helper は、attempt とattempt の間の経過時間だけでなく、各 attempt が呼ぶ I/O 自体 (fetch、body 読み取り) も明示的に bound しなければ、実際には unbounded になり得る**。`await fetchImpl(...)` に timeout や `AbortSignal` を渡さないまま「timeoutMs で bound している」と主張するコメントは、hang する実装や test double に対して簡単に嘘になる。この種の gap は deterministic な「fetch が resolve も reject もしない」regression test (injectable timer/clock で real wait なし) がないと検出しにくいため、bounded-retry helper には必ずそのようなテストを対にする。
