...
### LL-114: ページごとに max-width をハードコードするとレイアウトが drift する — 基底ルールから継承して単一ソース化
- **事象**: ユーザーから「記事ページ (`/e/[id]`) と Dashboard ページ (`/`) でサイドバーと中央パネルの幅が違う。記事ページの方 (中央パネルが広い) に合わせて」と指摘。ワイド画面で Dashboard だけ狭く窮屈に見えていた。
- **根本原因**: 両ページは同じ `.layout` (max-width **1440px**、3 カラム `232px 1fr 280px`) を使うが、Dashboard は `.layout home-layout` で `.home-layout` が **max-width を 1180px に上書き**していた (記事は右 rail があり 3 カラム、Dashboard は右 rail 無しの 2 カラム `232px minmax(0,1fr)`)。`.home-layout` は後勝ちで max-width だけ 1180px に絞っていたため、ワイド画面で Dashboard が **中央寄せ + 大きな左右余白**になり、サイドバーの左端位置も記事ページ (左寄せ、全幅) とずれていた。Playwright 実測では Dashboard の main カラム自体は記事より広かった (1440px 時 868 vs 824) が、**canvas 全体が 1180px に制限され中央寄せ**だったので「記事の方が中央パネルが広い」という体感が視覚的に正しかった (ユーザーの前提は列幅では逆だが、canvas 幅・整列では正しい)。2 ページが各自 max-width を持つ構造が drift の温床だった。
- **対策**: `.home-layout` から `max-width: 1180px` を**削除**し、`.layout` の 1440px を継承させた (`grid-template-columns: 232px minmax(0,1fr)` の 2 カラムは維持)。ハードコードで 1440 を書くのでなく**継承**にしたのは、両ページの canvas 幅を単一ソース (`.layout`) に寄せ、将来の再 drift を構造的に防ぐため。Playwright で 1512/1440/1280/1100/760px を実測し、両ページが同一 canvas 幅 (1440)・同一サイドバー幅 (232)・同一サイドバー左位置に揃い、Dashboard の中央パネルが広がり (1128px、右 rail が無いぶん記事の 824 より広い)、全幅で横スクロール無し、モバイル (760) では両者とも単一カラム 704 で一致することを確認。build + E2E 28 件 PASS。注: ≤1100px ではサイドバーが Dashboard 200 / 記事 188 とわずかに差が残るが、これは記事が右 rail を持つため `:has(aside.right)` の 3 カラムルール (901-1180px → `188px 1fr 220px`) にマッチするのに対し、Dashboard は右 rail が無く `200px 1fr` を使う**構造的差**で、ワイド画面 (≥1180) では完全一致するので主訴は解消。
- **教訓**: 同じ基底レイアウトを共有する複数ページが**各自 max-width / グリッドをハードコード**すると、片方の変更でもう片方と drift する (LL-022/072 の「単一情報源」族)。共有すべき寸法 (canvas 幅) は基底クラス (`.layout`) に置き、ページ固有の差分 (列構成・右 rail 有無) だけを override する。「A ページの方が良い」と言われたら、まず**両者の実寸を Playwright で測り**、体感差の真因 (列幅か、canvas 幅か、整列か) を切り分ける — ユーザーの言う「広い/狭い」は列幅でなく canvas 全体の幅・中央寄せ・左端整列に起因することがある (印象と実測の乖離、R-021/§4.3)。レイアウト幅の修正は単一 viewport でなくブレークポイント前後の複数幅で grid 列数・横スクロール・左端位置を実測する (LL-091)。
