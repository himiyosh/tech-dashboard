...
- PR merge の exact-head 独立レビュー gate が拒否した場合、valid・stale・wrong reviewer・self-issued・malformed marker の件数を 1 行で診断できるようにしました。JSON・GitHub API・evidence normalization が結果生成前に失敗した場合は件数を生成しません。
