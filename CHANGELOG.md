...
- 記事末尾のAI対話を、ポコの選ばれた非公開デザイン参照の特徴から描き起こした試作SVGと、TECH Dashboard 独自の「TECHガイド」で見せるようにしました。参照画像そのものはサイトへ配置しません。スマートフォンでも両方のキャラクターを表示し、対話の読み幅を保ちます。既存の保存済み台本と記事の事実は変更せず、旧配役への明らかな呼びかけのみ表示時に調整します。今後の対話生成も新配役に合わせます。
