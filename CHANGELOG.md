...
- 広い Zenn AI feed の Local LLM 判定を、`Ollama`、明示model/runtime、`ローカルLLM` / `ローカルAI` などタイトル上の主題根拠へ限定し、本文中または題名中のbare「ローカル」だけを理由に、業務・DB・一般AIデモが `local-llm` として掲載されないようにしました。既存の live/archive data にも同じ source filter を再適用しています。
